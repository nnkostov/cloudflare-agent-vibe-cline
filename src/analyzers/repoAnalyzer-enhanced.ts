...
export class RepoAnalyzerEnhanced extends BaseService {
  ...
  private async calculateEnhancedFactors(
    ...
    if (metrics.commits && metrics.commits.length > 0) {
      ...
      factors.commitFrequency =
        recentCommits.reduce((sum, m) => sum + m.commit_count, 0) / 30;
      factors.uniqueAuthors = Math.max(
        ...recentCommits.map((m) => m.unique_authors),
      );
      factors.codeChurn = recentCommits.reduce(
        (sum, m) => sum + m.additions + m.deletions,
        0,
      );
    }
    ...
    if (metrics.releases && metrics.releases.length > 0) {
      ...
      factors.latestReleaseAge = Math.floor(
        (Date.now() - new Date(metrics.releases[0].published_at).getTime()) /
          (1000 * 60 * 60 * 24),
      );
      factors.totalDownloads = metrics.releases.reduce(
      ...
      if (metrics.releases.length > 1) {
        ...
        const monthsSpan = Math.max(
          1,
          (Date.now() - new Date(oldestRelease.published_at).getTime()) /
            (1000 * 60 * 60 * 24 * 30),
        );
    ...
    if (metrics.pullRequests) {
      ...
      factors.prMergeRate =
        metrics.pullRequests.merged_prs /
        Math.max(1, metrics.pullRequests.total_prs);
      factors.prContributors = metrics.pullRequests.unique_contributors;
      factors.prResponseTime =
        metrics.pullRequests.avg_time_to_merge_hours || 168; // Default to 1 week
    }

    // Issue metrics
    if (metrics.issues) {
      factors.issueActivity = metrics.issues.total_issues;
      factors.issueCloseRate =
        metrics.issues.closed_issues / Math.max(1, metrics.issues.total_issues);
      factors.issueResponseTime =
        metrics.issues.avg_time_to_first_response_hours || 48; // Default to 2 days
      factors.bugRatio =
        metrics.issues.bug_issues / Math.max(1, metrics.issues.total_issues);
    }
    ...
    if (metrics.stars && metrics.stars.length > 0) {
      ...
      const weekAgoStars =
        recentStars[recentStars.length - 1]?.star_count || repo.stars;
      factors.weeklyStarGrowth = repo.stars - weekAgoStars;
      factors.starVelocity = factors.weeklyStarGrowth / 7;

      // Calculate growth rate
      if (weekAgoStars > 0) {
        factors.starGrowthRate =
          ((repo.stars - weekAgoStars) / weekAgoStars) * 100;
      }
    }

    // Fork network metrics
    if (metrics.forks) {
      factors.activeForkRatio =
        metrics.forks.active_forks / Math.max(1, metrics.forks.total_forks);
      factors.forkEngagement =
        metrics.forks.forks_with_stars / Math.max(1, metrics.forks.total_forks);
  ...
  private calculateGrowthScore(factors: Record<string, number>): number {
    ...
    const contributorScore = Math.min(
      100,
      (factors.uniqueAuthors || 0) * 10 + (factors.prContributors || 0) * 5,
    );
  ...
  private calculateEngagementScore(factors: Record<string, number>): number {
    ...
    // Issue engagement (25%)
    const issueEngagement = Math.min(
      100,
  ...
  calculateEngagementScoreForTier(metrics: {
    ...
    const forkRatio =
      metrics.forks / Math.max(1, metrics.forks + metrics.issues);
    score += forkRatio * 40;
