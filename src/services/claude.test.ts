...
describe('ClaudeService', () => {
  ...
  describe('isClaudeV4Model', () => {
    it('should identify Claude 4 models correctly', () => {
      ...
      expect(serviceAny.isClaudeV4Model('claude-opus-4')).toBe(true);
      expect(serviceAny.isClaudeV4Model('claude-sonnet-4')).toBe(true);
      expect(serviceAny.isClaudeV4Model('claude-3-5-sonnet-20241022')).toBe(true);
      expect(serviceAny.isClaudeV4Model('claude-3-5-sonnet-20240620')).toBe(true);
      expect(serviceAny.isClaudeV4Model('claude-3-haiku-20240307')).toBe(false);
      expect(serviceAny.isClaudeV4Model('claude-3-opus-20240229')).toBe(false);
    });
  });
  ...
  describe('getMaxTokensForModel', () => {
    it('should return correct token limits for each model type', () => {
      ...
      expect(serviceAny.getMaxTokensForModel('claude-opus-4')).toBe(4096);
      expect(serviceAny.getMaxTokensForModel('claude-sonnet-4')).toBe(8192);
      expect(serviceAny.getMaxTokensForModel('claude-3-haiku-20240307')).toBe(4096);
      expect(serviceAny.getMaxTokensForModel('claude-3-5-sonnet-20241022')).toBe(8192);
    });
  });
  ...
  describe('estimateCost', () => {
    it('should calculate correct costs for each model', () => {
      ...
      expect(serviceAny.estimateCost('claude-opus-4', responseLength)).toBeCloseTo(0.015, 3);
      expect(serviceAny.estimateCost('claude-sonnet-4', responseLength)).toBeCloseTo(0.003, 3);
      
      // Test other models
      expect(serviceAny.estimateCost('claude-3-haiku-20240307', responseLength)).toBeCloseTo(0.00025, 5);
    });
  });

  describe('analyzeRepository', () => {
    it('should analyze repository with claude-opus-4 model', async () => {
      const mockResponse = {
        content: [{
          type: 'text',
          text: JSON.stringify({
      ...
      const result = await service.analyzeRepository(mockRepo, mockReadme, 'claude-opus-4');
    ...
    it('should analyze repository with claude-sonnet-4 model', async () => {
      const mockResponse = {
        content: [{
          type: 'text',
          text: JSON.stringify({
      ...
      const result = await service.analyzeRepository(mockRepo, mockReadme, 'claude-sonnet-4');
    ...
    it('should handle API errors gracefully', async () => {
      ...
      await expect(
        service.analyzeRepository(mockRepo, mockReadme, 'claude-opus-4')
    ...
    it('should use enhanced prompt for V4 models', async () => {
      ...
      await service.analyzeRepository(mockRepo, mockReadme, 'claude-opus-4');
    ...
    it('should use standard prompt for non-V4 models', async () => {
      ...
      await service.analyzeRepository(mockRepo, mockReadme, 'claude-3-haiku-20240307');

      const callBody = JSON.parse(mockFetch.mock.calls[0][1].body);
      const prompt = callBody.messages[0].content;
      
      // Standard prompt characteristics
      expect(prompt).toContain('venture capital analyst');
      expect(prompt).not.toContain('technical_moat');
      expect(prompt).toContain('5000'); // Standard README character limit
    }, 10000);
  });
  ...
  describe('parseResponse', () => {
    it('should handle malformed JSON gracefully', async () => {
      ...
      await expect(
        service.analyzeRepository(mockRepo, mockReadme, 'claude-opus-4')
      ).rejects.toThrow('Failed to parse analysis');
    }, 10000);
