        ...
        return globalConnectionPool.withConnection(async () => {
          return withExponentialBackoff(async () => {
            try {
              ...
              console.error(
                `Error getting commit activity for ${owner}/${repo}:`,
                error,
              );
            ...
            try {
              ...
              return {
                ...
                avg_time_to_merge_hours:
                  mergedWithTime > 0 ? totalMergeTime / mergedWithTime : null,
              ...
              console.error(
                `Error getting PR metrics for ${owner}/${repo}:`,
                error,
              );
              throw error; // Re-throw for exponential backoff
            }
      ...
      return withExponentialBackoff(async () => {
        try {
          ...
          for (const issue of realIssues) {
            ...
            if (
              labels.some(
                (l) => l.includes("feature") || l.includes("enhancement"),
              )
            ) {
              featureIssues++;
            }
          ...
          return {
            ...
            avg_time_to_close_hours:
              closedWithTime > 0 ? totalCloseTime / closedWithTime : null,
          ...
          console.error(
            `Error getting issue metrics for ${owner}/${repo}:`,
            error,
          );
          throw error; // Re-throw for exponential backoff
        }
    ...
    return globalConnectionPool.withConnection(async () => {
      return withExponentialBackoff(async () => {
        try {
          ...
          console.error(
            `Error getting star history for ${owner}/${repo}:`,
            error,
          );
          throw error; // Re-throw for exponential backoff
        }
      ...
      return withExponentialBackoff(async () => {
        try {
          ...
          return {
            ...
            avg_fork_stars:
              forksWithStars > 0 ? totalForkStars / forksWithStars : null,
          };
        } catch (error) {
          console.error(
            `Error analyzing fork network for ${owner}/${repo}:`,
            error,
          );
          throw error; // Re-throw for exponential backoff
        }
        ...
        for (const strategy of strategies) {
          ...
          await globalConnectionPool.withConnection(async () => {
            await withExponentialBackoff(async () => {
              try {
                console.log(
                  `Searching with strategy: ${strategy.type} - ${strategy.query}`,
                );
                ...
                for (const repo of data.items) {
                  if (!allRepos.has(repo.id.toString())) {
                    allRepos.set(
                      repo.id.toString(),
                      this.mapGitHubRepoToRepository(repo),
                    );
                  }
                }
              } catch (error) {
                console.error(
                  `Error with search strategy ${strategy.type}:`,
                  error,
                );
                throw error; // Re-throw for exponential backoff
              }
