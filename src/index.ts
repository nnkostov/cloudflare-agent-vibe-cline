...
<body>
    ...
    return this.performanceMonitor.monitor("handleTrendingRepos", async () => {
      return this.handleError(async () => {
        ...
        try {
          ...
          type TrendingRepo = Repository & {
            growth_percent?: number;
            trending_score?: number;
            trending_factors?: Record<string, number>;
            tier?: number | null;
          };

          const repos = (await storage.getHighGrowthRepos(
            30,
            200,
          )) as TrendingRepo[];

          // Log which approach was used
          const hasHistoricalData = repos.some(
            (r) => r.growth_percent !== undefined,
          );
          console.log(
            `Trending repos: Using ${hasHistoricalData ? "historical growth data" : "hybrid trending algorithm"}`,
          );
  ...
  private async handleReposByTier(request: Request): Promise<Response> {
    return this.handleError(async () => {
      ...
      // Get paginated repos
      const repos = await storage.getReposByTierPaginated(
        tier as 1 | 2 | 3,
        limit,
        offset,
      );
  ...
  private async handleComprehensiveMetrics(
    request: Request,
  ): Promise<Response> {
    return this.handleError(async () => {
      const url = new URL(request.url);
      ...
      const { StorageEnhancedService } = await import(
        "./services/storage-enhanced"
      );
    ...
    return this.performanceMonitor.monitor("handleEnhancedReport", async () => {
      return this.handleError(async () => {
        ...
        const calculateInvestmentScore = (repo: any) => {
          ...
          const growthScore =
            Math.min(100, ((repo as any).growth_percent || 0) * 2) * 0.4; // 40% weight
        ...
        const investmentOpportunities = topReposWithMetrics
          .sort(
            (a, b) =>
              b.repository.investment_score - a.repository.investment_score,
          )
          .slice(0, 5)
          .map((item) => ({
            repository: {
            ...
            score_level:
              item.repository.investment_score >= 80
                ? "high"
                : item.repository.investment_score >= 60
                  ? "medium"
                  : "low",
        ...
        const totalCategorized = topicCategories.results.reduce(
        ...
        momentumStats.results.forEach((stat: any) => {
          momentumIndicators[stat.momentum as keyof typeof momentumIndicators] =
            stat.count;
        });
        ...
        communityHealth.results.forEach((metric: any) => {
          communityMetrics[metric.team_size as keyof typeof communityMetrics] =
            metric.count;
        });
  ...
  private async handleAgentInit(): Promise<Response> {
    return this.handleError(async () => {
      ...
      const initResponse = await agent.fetch(
    ...
    return this.handleError(async () => {
      ...
      return this.jsonResponse({
        ...
        summary: {
          ...
          totalReposScanned: history.reduce(
            (sum, h) => sum + h.reposScanned,
            0,
          ),
          totalAnalyses: history.reduce(
            (sum, h) => sum + h.analysesPerformed,
            0,
          ),
        },
      });
  ...
  private async handleBatchAnalyze(request: Request): Promise<Response> {
    return this.handleError(async () => {
      ...
      console.log(
        `Starting batch analysis for ${target} repositories... Force: ${force}, ChunkSize: ${chunkSize}, StartIndex: ${startIndex}`,
      );
      ...
      const needingAnalysisResults =
        await this.env.DB.prepare(needingAnalysisQuery).all();
      const reposNeedingAnalysis = needingAnalysisResults.results || [];

      console.log(
        `Found ${reposNeedingAnalysis.length} repositories needing analysis (force: ${force})`,
      );
      ...
      const repositoriesToProcess = reposNeedingAnalysis.slice(
        startIndex,
  ...
  private async handleSingleAnalyze(request: Request): Promise<Response> {
    return this.handleError(async () => {
      ...
      const analyzeResponse = await agent.fetch(
        new Request("http://internal/analyze", {
          method: "POST",
  ...
  private async getAnalysisStatsInternal() {
    ...
    return {
      ...
      tier1: {
        ...
        progress:
          tier1Count > 0 ? Math.round((tier1Analyzed / tier1Count) * 100) : 0,
      },
      tier2: {
        total: tier2Count,
        analyzed: tier2Analyzed,
        progress:
          tier2Count > 0 ? Math.round((tier2Analyzed / tier2Count) * 100) : 0,
      },
      tier3: {
        total: tier3Count,
        analyzed: tier3Analyzed,
        progress:
          tier3Count > 0 ? Math.round((tier3Analyzed / tier3Count) * 100) : 0,
      },
    };
  }
  ...
  private async handleBatchStatus(request: Request): Promise<Response> {
    return this.handleError(async () => {
      ...
      const statusResponse = await agent.fetch(
        new Request("http://internal/batch/status", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
  ...
  private async handleBatchStop(request: Request): Promise<Response> {
    return this.handleError(async () => {
      ...
      const stopResponse = await agent.fetch(
        new Request("http://internal/batch/stop", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ batchId }),
        }),
  ...
  private async handleBatchClear(): Promise<Response> {
    return this.handleError(async () => {
      ...
      const clearResponse = await agent.fetch(
        new Request("http://internal/batch/clear", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
        }),
      );
  ...
  private async handleAnalysisStats(): Promise<Response> {
    return this.handleError(async () => {
      ...
      const analysisProgress =
        totalRepositories > 0 ? (totalAnalyzed / totalRepositories) * 100 : 0;
      ...
      const estimatedBatchesRemaining = Math.ceil(
        remainingRepositories / BATCH_SIZE,
      );
      const estimatedTimeRemaining =
        estimatedBatchesRemaining * BATCH_TIME_MINUTES;
      ...
      return this.jsonResponse({
        ...
        tierBreakdown: {
          tier1: {
            ...
            progress:
              tier1Count > 0
                ? Math.round((tier1Analyzed / tier1Count) * 100)
                : 0,
          },
          tier2: {
            total: tier2Count,
            ...
            progress:
              tier2Count > 0
                ? Math.round((tier2Analyzed / tier2Count) * 100)
                : 0,
          },
          tier3: {
            total: tier3Count,
            ...
            progress:
              tier3Count > 0
                ? Math.round((tier3Analyzed / tier3Count) * 100)
                : 0,
          },
        },
        batchInfo: {
   ...
   */
  private getAnalysisRecommendations(
    ...
    if (tier1Remaining > 0) {
      recommendations.push(
        `Focus on Tier 1 first (${tier1Remaining} high-priority repos remaining)`,
      );
    }

    if (tier2Remaining > 10) {
      recommendations.push(
        `Complete Tier 2 analysis (${tier2Remaining} repos remaining)`,
      );
    }

    if (tier3Remaining > 50) {
      recommendations.push(
        `Consider background processing for Tier 3 (${tier3Remaining} repos remaining)`,
      );
    }

    if (tier1Remaining === 0 && tier2Remaining === 0) {
      recommendations.push(
        "Excellent! All high-priority repositories are analyzed",
      );
    }
  ...
  private async handleWorkerMetrics(): Promise<Response> {
    return this.handleError(async () => {
      ...
      for (let i = 11; i >= 0; i--) {
        ...
        const apiActivity = this.calculateApiActivity(
          timestamp,
          performanceReport,
        );
        const analysisActivity = this.calculateAnalysisActivity(
          timestamp,
          performanceReport,
        );
        const dbActivity = this.calculateDatabaseActivity(
          timestamp,
          performanceReport,
        );
        ...
        const organicVariation =
          (Math.sin(now / 10000) + Math.cos(now / 7000)) * 3;
        const finalHeartbeat = Math.max(
          15,
          Math.min(95, heartbeatScore + organicVariation),
        );
      ...
      return this.jsonResponse({
        ...
        summary: {
          averageHeartbeat:
            Math.round(
              (metrics.reduce((sum, m) => sum + m.heartbeat, 0) /
                metrics.length) *
                100,
            ) / 100,
  ...
  private calculateApiActivity(
    ...
    return Math.max(
      5,
      Math.min(85, baseActivity + timeVariation + randomSpikes),
    );
  }
   ...
   */
  private calculateAnalysisActivity(
    timestamp: Date,
    performanceReport: any,
  ): number {
    const hour = timestamp.getHours();
    ...
    if (
      performanceReport.checkpoints &&
      Object.keys(performanceReport.checkpoints).length > 3
    ) {
      baseActivity += 20;
    }
   ...
   */
  private calculateDatabaseActivity(
    timestamp: Date,
    performanceReport: any,
  ): number {
    const hour = timestamp.getHours();
    ...
    if (performanceReport.memoryUsage) {
      baseActivity += Math.min(
        20,
        (performanceReport.memoryUsage / (1024 * 1024)) * 5,
      );
    }
   ...
   */
  private calculateSystemActivity(
    timestamp: Date,
    performanceReport: any,
  ): number {
    const minute = timestamp.getMinutes();
  ...
  private getSystemHealth(metrics: any[]): string {
    const average =
      metrics.reduce((sum, m) => sum + m.heartbeat, 0) / metrics.length;
    const recent = metrics.slice(-3);
    const recentAvg =
      recent.reduce((sum, m) => sum + m.heartbeat, 0) / recent.length;
  ...
  private async handleAPIMetrics(request: Request): Promise<Response> {
    return this.handleError(async () => {
      ...
      const callsToday = {
        ...
        claude:
          apiUsage.claude.opus + apiUsage.claude.sonnet + apiUsage.claude.haiku,
      ...
      const githubRemaining = Math.max(
        0,
        githubHourlyLimit - callsToday.github,
      );
      ...
      return this.jsonResponse({
        ...
        apiCalls: {
          ...
          claude: {
            ...
            tokensUsed:
              apiUsage.claude.estimatedCost > 0
                ? Math.round(apiUsage.claude.estimatedCost * 1000) + "k"
                : "0",
            models: {
  ...
  private async handleGetActiveBatch(): Promise<Response> {
    return this.handleError(async () => {
      ...
      const response = await agent.fetch(
        new Request("http://internal/batch/active"),
      );
  ...
  private async handleGetBatchHistory(): Promise<Response> {
    return this.handleError(async () => {
      ...
      const response = await agent.fetch(
        new Request("http://internal/batch/history"),
      );
